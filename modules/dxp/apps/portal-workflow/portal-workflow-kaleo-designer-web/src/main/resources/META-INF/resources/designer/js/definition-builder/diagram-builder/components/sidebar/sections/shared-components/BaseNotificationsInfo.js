...
const BaseNotificationsInfo = ({
	...
	const [executionType, setExecutionType] = useState(
		pathDataNotificationsInfo?.executionType?.[notificationIndex] ||
	...
	const [notificationDescription, setNotificationDescription] = useState(
		pathDataNotificationsInfo?.description?.[notificationIndex] || ''
	);
	const [notificationName, setNotificationName] = useState(
		pathDataNotificationsInfo?.name?.[notificationIndex] || ''
	);
	const [notificationTypeEmail, setNotificationTypeEmail] = useState(
		pathDataNotificationsInfo?.notificationTypes?.[
			notificationIndex
	...
	const [recipientType, setRecipientType] = useState(
		getRecipientType(
			pathDataNotificationsInfo?.recipients?.[notificationIndex]
				?.assignmentType?.[0]
		) || 'assetCreator'
	);
	const [template, setTemplate] = useState(
		pathDataNotificationsInfo?.template?.[notificationIndex] || ''
	);
	const [templateLanguage, setTemplateLanguage] = useState(
		pathDataNotificationsInfo?.templateLanguage?.[notificationIndex] ||
			'freemarker'
	);
						...
						<ClayForm.Group className="recipient-type-form-group">
							{internalSections.map(({identifier}, index) => (
								<RecipientTypeComponent
									...
									index={index}
									inputValue={
										pathDataNotificationsInfo?.recipients[
											notificationIndex
										]?.script?.[0]
									}
